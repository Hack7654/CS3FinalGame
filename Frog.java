...
public class Frog extends JPanel implements KeyListener{
	...
	Frog(int x,int y,int w,int h){
		...
		this.w=w;
		this.h=h;
		frog=new Rectangle(x,y,w-6,h);
		changeSprite("img/B.png");
	...
	public void keyPressed(KeyEvent e) {
		if(state()){
			...
			if (key==KeyEvent.VK_RIGHT) {
				if(frog.getMaxX()+1*Display.GRID<Display.WIDTH&&sprite=="img/R.png"){
					frog.x=frog.x+1*Display.GRID;
					frog.setLocation(frog.x, frog.y);			
				}
				else{
					changeSprite("img/R.png");
				}
			}
			else if(key==KeyEvent.VK_LEFT) {
				if(frog.getMaxX()-1*Display.GRID>0&&sprite=="img/L.png"){
					frog.x=frog.x-1*Display.GRID;
					frog.setLocation(frog.x, frog.y);
				}
				else{
					changeSprite("img/L.png");
				}
			}
         ...
         {
            if(frog.y + Display.GRID < Display.HEIGHT - 10) // Adjusted condition
               frog.y = frog.y + Display.GRID;
            if(frog.x % 50 == 0)
               frog.setLocation(frog.x, frog.y);
            else
            {
