	...
	public void keyPressed(KeyEvent e) {
		if(state()){
			...
			if (key==KeyEvent.VK_RIGHT) {
				if(frog.getMaxX()+1*Display.GRID<Display.WIDTH&&sprite=="img/Right1.png"){
					frog.x=frog.x+1*Display.GRID;
					frog.setLocation(frog.x, frog.y);			
				}
				else{
					changeSprite("img/Right1.png");
				}
			}
			else if(key==KeyEvent.VK_LEFT) {
				if(frog.getMaxX()-1*Display.GRID>0&&sprite=="img/Left1.png"){
					frog.x=frog.x-1*Display.GRID;
					frog.setLocation(frog.x, frog.y);
				}
				else{
					changeSprite("img/Left1.png");
				}
			}
